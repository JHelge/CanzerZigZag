...
#SBATCH --job-name=CRC_bio_trajectory
...
SCRIPT=/prj/ml-ident-canc/original_codes/scDiffusion/zigzag_refactored_safe_chat/analysis/zigzag_biological_trajectory_analysis.py
...
python -u "$SCRIPT" \
  ...
  --top_pathways_per_trajectory 12 \
  --top_pathways_per_mode 8 \
  --top_genes 60 \
  --max_modes_per_seed 4 \
  --sparsity_project \
